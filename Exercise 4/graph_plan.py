...
def independent_pair(a1, a2):
    ...
    a1_propositions = a1.get_add()
    a2_propositions = a2.get_add()

    for prop in a1_propositions:
        if a2.is_neg_effect(prop):
            return False
    for prop in a2_propositions:
        if a1.is_neg_effect(prop):
            return False

    # interference:
    a1_precondition = a1.get_pre()
    a2_precondition = a2.get_pre()

    for pre in a1_precondition:
        if a2.is_neg_effect(pre):
            return False
    for pre in a2_precondition:
